...
...
 ...
 Welcome to Zangado - the low bandwidth, yet greedy and poisonous HTTP client
EOTEXT
...
EOSUCKS2BU
    ...
    print
"Connectando a Host $host:$port every $timeout seconds with $connections sockets:\n  By AngelHacker";
...
sub doconnections {
    ...
    while (1) {
        $failedconnections = 0;
        print "\t\tDerrubando site by Zangado.\n";
        foreach my $z ( 1 .. $num ) {
            if ( $working[$z] == 0 ) {
                if ($ssl) {
                    if (
...
SAngel both helps identify the timeout windows of a HTTP server or Proxy server, can bypass httpready protection and ultimately performs a fairly low bandwidth denial of service.  It has the added benefit of allowing the server to come back at any time (once the program is killed), and not spamming the logs excessively.  It also keeps the load nice and low on the target server, so other vital processes don't die unexpectedly, or cause alarm to anyone who is logged into the server for other reasons.
...
=head1 DESCRIPTIONHeLL is designed so that a single machine (probably a Linux/UNIX machine since Windows appears to limit how many sockets you can have open at any given time) can easily tie up a typical web server or proxy server by locking up all of it's threads as they patiently wait for more data.  Some servers may have a smaller tolerance for timeouts than others, but HrLL can compensate for that by customizing the timeouts.  There is an added function to help you get started with finding the right sized timeouts as well.
